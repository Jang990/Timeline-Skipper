...
- **e2e는 진짜 브라우저가 있어야만 확인되는 것만 맡는다.** 테스트마다 크로미움을 띄우는
  비용이 커서, jsdom으로 확인되는 것을 e2e에 두지 않는다.
  - 색·위치·크기·겹침 (`boundingBox`, 계산된 스타일, hover)
  - 실제 영상 재생과의 연동, 새로고침 뒤에도 유지되는지
  - 진짜 초점 이동 (예: 메뉴 밖 클릭으로 닫기)
  - 기능마다 대표 흐름 하나
- `adapters/`는 픽스처 페이지 위에서 도는 e2e로 검증한다.
