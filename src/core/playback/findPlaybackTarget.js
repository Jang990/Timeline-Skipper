...
export function findPlaybackTarget({ tracks, disabledStartSeconds, loopEnabled, isEditing, isTurnedOff }, currentTimeSeconds) {
  // 편집 중에는 곡 경계를 들으며 맞춘다. 해제된 트랙이나 빈 구간을 들여다보는 순간 튕겨 나가면 맞출 수가 없다.
  // 꺼 두었으면 영상은 사람이 튼 그대로 흘러야 한다.
  if (isEditing || isTurnedOff) {
    return null
  }
