...
test.describe('따라가는 동안의 재생', () => {
  test('따라가기를 켜지 않은 편집 중에는 해제된 트랙을 그대로 건너뛴다', async ({ openWatchPage }) => {
  ...
  test('따라가는 동안에는 해제된 트랙으로 옮겨도 그 자리에 머문다', async ({ openWatchPage }) => {
    const page = await openTimeline(openWatchPage)
    await disableTrack(page, '둘째 곡')
    await openEditRow(page, '둘째 곡')
  ...
  test('따라가는 동안에는 반복이 켜져 있어도 마지막 트랙 끝에서 되감지 않는다', async ({ openWatchPage }) => {
    const page = await openTimeline(openWatchPage)
  ...
  test('편집을 취소하면 따라가기가 꺼져 다시 해제된 트랙을 건너뛴다', async ({ openWatchPage }) => {
    const page = await openTimeline(openWatchPage)
    await disableTrack(page, '둘째 곡')
    await openEditRow(page, '둘째 곡')
    await followButton(page).click()
