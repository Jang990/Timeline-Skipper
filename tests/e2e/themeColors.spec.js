import { test, expect } from '../fixtures/extensionContext.js'
import { readCommentSnapshot } from '../fixtures/snapshots/commentSnapshot.js'

// 트랙 시작은 1, 269, 556, 810, 1026, 1253, 1508, 1795초다. 1100초는 다섯 번째 트랙 안이다.
const { commentTexts } = readCommentSnapshot('3yG8GXdnEFQ')

const PANEL = '#timeline-skip-panel'
const ROW = `${PANEL} .timeline-skip-row`

// 유튜브는 테마를 바꿀 때 html에 이 변수들을 다시 정한다. 픽스처에는 유튜브 스타일이 없어서 직접 정한다.
// 어느 테마의 값과도 겹치지 않는 색을 써야, 대체값이 아니라 이 값을 따랐다는 것이 드러난다.
const TEXT_PRIMARY = 'rgb(1, 2, 3)'
const CALL_TO_ACTION = 'rgb(4, 5, 6)'

test.describe('테마 색', () => {
  test('페이지가 유튜브 글자색 변수를 정하면 트랙 제목이 그 색을 따른다', async ({ openWatchPage }) => {
    const { page } = await openWatchPage({ commentTexts })
    await loadTimeline(page)

    await setPageVariable(page, '--yt-sys-color-baseline--text-primary', TEXT_PRIMARY)

    await expect(page.locator(`${ROW} .timeline-skip-title`).first()).toHaveCSS('color', TEXT_PRIMARY)
  })

  test('페이지가 유튜브 강조색 변수를 정하면 재생 중인 행의 시각이 그 색을 따른다', async ({ openWatchPage }) => {
    const { page } = await openWatchPage({ commentTexts })
    await loadTimeline(page)
    await seekAndSettle(page, 1100)

    await setPageVariable(page, '--yt-sys-color-baseline--call-to-action', CALL_TO_ACTION)

    await expect(page.locator(`${ROW}.is-playing .timeline-skip-time`)).toHaveCSS('color', CALL_TO_ACTION)
  })
})

function setPageVariable(page, name, value) {
  return page.evaluate(([variableName, variableValue]) => {
    document.documentElement.style.setProperty(variableName, variableValue)
  }, [name, value])
}

// 탐색은 영상 길이를 안 뒤에만 먹힌다. 불러오기 전에 그것부터 기다린다.
async function loadTimeline(page) {
  await expect
    .poll(() => page.evaluate(() => document.querySelector('video').readyState))
    .toBeGreaterThanOrEqual(1)

  await page.locator('.timeline-skip-load-button').first().click()
  await expect(page.locator(ROW).first()).toBeVisible()
}

function seekAndSettle(page, timestampSeconds) {
  return page.evaluate(
    (seconds) =>
      new Promise((resolve) => {
        const video = document.querySelector('video')

        video.addEventListener('timeupdate', () => resolve(video.currentTime), { once: true })
        video.currentTime = seconds
      }),
    timestampSeconds
  )
}
