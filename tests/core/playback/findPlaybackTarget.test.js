...
function playbackOf(overrides) {
  return { tracks, disabledStartSeconds: new Set(), loopEnabled: false, isFollowing: false, ...overrides }
}

describe('findPlaybackTarget', () => {
  it('해제된 트랙 안이면 다음 켜진 트랙으로 옮긴다', () => {
  ...
  it('따라가는 동안에는 해제된 트랙 안이어도 옮기지 않는다', () => {
    const playback = playbackOf({ disabledStartSeconds: new Set([100]), isFollowing: true })
  ...
  it('따라가는 동안에는 빈 구간이어도 옮기지 않는다', () => {
    const playback = playbackOf({ tracks: tracksWithGap, isFollowing: true })
  ...
  it('따라가는 동안에는 반복이 켜져 있어도 되감지 않는다', () => {
    const playback = playbackOf({ tracks: tracksEndingEarly, loopEnabled: true, isFollowing: true })
